...
from multimodal import MultimodalMode
from metrics import (
    record_llm_call,
...
class PostgresCheckpointer:
    ...
    def get_state(self, config: dict):
        """获取保存的状态"""
        return self.initialize().get_state(config)

    @_postgres_safe_call
    def put_state(self, config: dict, state: dict):
        """保存状态"""
        saver = self.initialize()
        saver.put_state(config, state)

    @_postgres_safe_call
    def invoke(self, input_state: dict, config: dict):
        """执行工作流"""
        saver = self.initialize()
        return saver.invoke(input_state, config)
...
def router_node(state: AgentState) -> AgentState:
    ...
    if not messages:
        ...
        return {**state, "intent": None, "cache_eligible": False}
    ...
    # ─────────────────────────────────────────────────────────────────
    # Step 3: 意图分类（使用 prompts.py 统一模板）
    # ─────────────────────────────────────────────────────────────────
    try:
        from prompts import intent_classify
        router_llm = _build_router_llm()
        router_model = getattr(settings, 'router_llm_model', 'gpt-4o-mini')
        ...
        try:
            messages = intent_classify(latest_message)
        ...
        # 关键设计：action 和 chitchat 意图不使用缓存（操作结果和闲聊不应该被缓存）
        cache_eligible = intent == "policy"
...
def chitchat_node(state: AgentState) -> AgentState:
    ...
    return {
        **state,
        "messages": state.get("messages", []) + [AIMessage(content=polite_response)],
    }
...
def rag_node(state: AgentState) -> AgentState:
    ...
    if multimodal_context:
        mode = multimodal_context.get("mode")

        if mode == MultimodalMode.REIMBURSEMENT.value:
            # 发票报销场景：检索报销政策（优先 LlamaIndex Pipeline）
            try:
                from rag import build_llama_rag_pipeline
                pipeline = build_llama_rag_pipeline()
                multimodal_docs = pipeline.retrieve(
                    query=f"报销标准 {invoice_type} {amount}元",
                    top_k=3,
                    use_rerank=False,
                )
            except Exception:
                # 降级：使用旧版 rag.py
                try:
                    from rag import build_graph_filtered_retriever
                    retriever = build_graph_filtered_retriever()
                    invoice_data = multimodal_context.get("invoice_data", {})
                    amount = invoice_data.get("total_amount") or invoice_data.get("amount", 0)
                    invoice_type = invoice_data.get("invoice_type", "通用发票")
                    policy_query = f"报销标准 {invoice_type} {amount}元"
                    multimodal_docs = retriever.retrieve(
                        query=policy_query,
                        user_id=user_id,
                        top_k=3,
                    )
            ...
            # 构建发票信息摘要
            invoice_summary = f"""【发票信息】
- 发票类型：{invoice_data.get('invoice_type', '未知')}
- 发票号码：{invoice_data.get('invoice_number', '未知')}
- 金额：{invoice_data.get('total_amount') or invoice_data.get('amount', '未知')}元
- 日期：{invoice_data.get('date', '未知')}
- 抬头：{invoice_data.get('title', '未知')}

请根据以上发票信息，结合公司报销政策，回答用户的问题。"""
            multimodal_answer_addition = invoice_summary

        elif mode == MultimodalMode.DIAGNOSIS.value:
            # IT 故障诊断场景：检索错误解决方案（优先 LlamaIndex Pipeline）
            try:
                from rag import build_llama_rag_pipeline
                pipeline = build_llama_rag_pipeline()
                error_code = multimodal_context.get("error_code", "")
                error_msg = multimodal_context.get("error_message", "")
                diagnosis_query = f"{error_code} {error_msg}"
                multimodal_docs = pipeline.retrieve(
            ...
            # 构建诊断信息摘要
            diagnosis_summary = f"""【故障诊断信息】
- 错误代码：{error_code or '未知'}
- 错误信息：{error_msg or '未知'}
- 建议修复步骤：
{chr(10).join(f"  {i+1}. {s}" for i, s in enumerate(multimodal_context.get("suggested_fixes", [])[:5]))}

请结合知识库中的相关解决方案，回答用户的问题。"""
            multimodal_answer_addition = diagnosis_summary
    ...
    try:
        ...
        # 融合多模态文档
        all_docs = result.docs or []
        if multimodal_docs:
            # 将多模态文档添加到结果前面（优先级更高）
            all_docs = multimodal_docs + all_docs
        ...
        logger.info(
            ...
            extra={
                ...
                "multimodal_doc_count": len(multimodal_docs),
                "quality_score": result.usefulness_score,
        ...
        # 降级处理：尝试使用备用搜索
        try:
            from degrader import get_degrade_strategy
            strategy = get_degrade_strategy()
            degraded_result = strategy.execute_policy_query(latest_message)

            rag_message = AIMessage(
                content=degraded_result["answer"] + "\n\n" + degraded_result.get("degraded_note", "")
            )

            logger.warning(
                "降级到备用搜索",
                extra={
                    "layer": degraded_result["layer"],
                    "confidence": degraded_result["confidence"],
                    "component": "rag",
                }
            )
            ...
            return {
                **state,
                ...
                "skipped_retrieval": False,
                "reflection_results": [],
                "needs_escalation": degraded_result["layer"] >= 3,
                "messages": [rag_message],
            }
            ...
            logger.error(
                "降级搜索也失败了",
                extra={"error": str(degrade_error), "component": "rag"}
            )
            rag_message = AIMessage(
...
def draft_action_node(state: AgentState) -> AgentState:
    ...
    try:
        from prompts import action_param_extract
        prompt_messages = action_param_extract(latest_message)
        ...
        if has_unfinished_draft and previous_action_type:
            ...
            prompt_messages[-1].content = latest_message + resume_context

        response = llm_with_timeout(prompt_messages, node="draft_action")
        import json
        content = response.content.strip()
        for line in content.split("\n"):
            line = line.strip()
            ...
            action_type = "unknown"
            extracted_params = {}
            confirmation_message = "请确认您的操作请求"
    except Exception:
        action_type = "unknown"
        extracted_params = {}
...
def generate_response_node(state: AgentState) -> AgentState:
    ...
    messages = state.get("messages", [])
    if not messages:
        return {**state, "final_response": "您好，有什么可以帮您？"}
