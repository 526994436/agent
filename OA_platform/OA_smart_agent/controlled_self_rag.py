...
节点1：检索路由（RetrievalRouter）
├── 作用：决定"要不要查资料"
├── 比喻：就像公司前台的智能分流
│         - 问"发票怎么贴" → 转到财务知识库
│         - 说"谢谢" → 礼貌回复，不需要查资料
│         - "我电脑有点卡" → 不太确定，交给 AI 判断
└── 特点：规则优先（90%），AI 辅助（10%）

节点2：有用性判断（UsefulnessChecker）
├── 作用：判断"检索到的资料有没有用"
├── 比喻：就像论文查重系统
│         - 相似度 > 0.3 → 有参考价值，保留
│         - 相似度 < 0.3 → 不相关，丢弃
...
class RetrievalDecision(str, Enum):
    ...
    三种情况：
    - MUST_RETRIEVE: 必须查（命中了必须检索的关键词）
    - SKIP_RETRIEVE: 不用查（命中了跳过检索的关键词，如问候语）
    - MAYBE_RETRIEVE: 模糊情况（交给 AI 判断）
    """
    MUST_RETRIEVE = "must_retrieve"   # 必须检索
    SKIP_RETRIEVE = "skip_retrieve"   # 跳过检索
    MAYBE_RETRIEVE = "maybe_retrieve"  # 模糊情况，LLM 判断
...
class RetrievalRouteResult:
    ...
    属性说明：
    - decision: 最终决策（必须检索/跳过/模糊）
    - reason: 判断理由（方便排查问题）
    - matched_keywords: 命中的关键词（展示为什么做这个决策）
    - ambiguity_score: 模糊程度评分（0-1，越高越难判断）
    """
    decision: RetrievalDecision
    reason: str
    matched_keywords: List[str] = field(default_factory=list)
    ambiguity_score: float = 0.0  # 0-1，越高越模糊
...
class RetrievalRouter:
    ...
    【设计理念】
    想象你去公司前台问问题：
    - 问"财务室在哪" → 前台直接告诉你，不用查资料
    - 问"发票怎么贴" → 前台说"请咨询财务部"，需要转接
    - 问"最近天气不错" → 前台微笑点头，不用查资料
    
    我们的检索路由就是这样的"智能前台"！
    
    【工作原理】
    1. 规则优先（处理 90% 的明确情况）
       - 命中"必须检索"关键词（如"报销"、"流程"）→ 立即检索
       - 命中"跳过检索"关键词（如"你好"、"谢谢"）→ 跳过检索
    
    2. LLM 辅助（处理剩余 10% 的模糊情况）
       - 只有当规则无法判断时才调用 AI
       - 大大减少 AI 调用次数，节省成本
    
    【配置参数】
    - must_keywords: 必须检索的关键词列表
    - skip_keywords: 跳过检索的关键词列表
    - llm_threshold: LLM 判断的阈值（模糊度超过这个值才调用 AI）
    """
    ...
    def __init__(
        ...
        参数说明：
        - must_keywords: 触发检索的关键词（如 ["报销", "请假", "流程", "制度"]）
        - skip_keywords: 跳过检索的关键词（如 ["你好", "谢谢", "再见"]）
        - llm_threshold: 模糊度阈值，超过这个值才调用 AI
        """
        from config import settings
        
        # 从配置读取默认关键词，也可以手动传入
        self.must_keywords = must_keywords or settings.retrieval_must_keywords
        self.skip_keywords = skip_keywords or settings.retrieval_skip_keywords
        self.llm_threshold = llm_threshold or settings.retrieval_llm_threshold

    def route(self, query: str) -> RetrievalRouteResult:
        """
        执行检索路由判断

        简化版：直接返回必须检索，不再做关键词判断和 LLM 调用。
        检索决策由上层意图分类统一控制。
        """
        # 直接返回必须检索，不再做额外判断
        return RetrievalRouteResult(
            decision=RetrievalDecision.MUST_RETRIEVE,
            reason="统一检索策略，由意图分类控制",
            matched_keywords=[],
            ambiguity_score=0.0,
        )
...
# =============================================================================
# 第三部分：节点2 - 有用性判断（UsefulnessChecker）
# =============================================================================
...
class UsefulnessChecker:
    ...
    def __init__(
        ...
        - min_results: 最少保留的文档数量（即使分数不够，也至少保留 1 份最好的）
        """
        from config import settings
        
        self.score_threshold = score_threshold or settings.rerank_score_threshold
        self.min_results = min_results or settings.rerank_min_results
    ...
    def check(
        ...
        # 遍历每份文档，根据分数决定保留还是丢弃
        filtered_docs = []
        low_score_count = 0  # 计数器：记录被丢弃的文档数量
        
        for doc in docs:
            # 获取文档的评分（可能叫 rerank_score 或 score）
        ...
        if len(filtered_docs) < self.min_results:
            ...
            if docs:
                ...
                low_score_count = max(0, len(docs) - len(filtered_docs))
                decision = UsefulnessDecision.USEFUL  # 破例通过
                reason = f"结果不足 {self.min_results}，补充低分文档"
            else:
                decision = UsefulnessDecision.NOT_USEFUL
                reason = f"文档数量不足（{len(docs)} < {self.min_results}）"
        else:
            # 正常情况：有足够的合格文档
        ...
        ) / len(filtered_docs) if filtered_docs else 0.0

        # 记录判断结果，方便后续分析和优化
        logger.info(
            "usefulness_check_completed",
            extra={
                "decision": decision.value,
                "threshold": self.score_threshold,
                "avg_score": avg_score,
                "filtered_count": low_score_count,  # 被丢弃的文档数
                "remaining_count": len(filtered_docs),  # 保留的文档数
...
class AnswerQualityChecker:
    ...
    def check(
        ...
        if not docs:
            # 无文档但有答案 → 可能是幻觉
            return self._check_without_docs(query, answer)

        return self._check_with_docs(query, answer, docs)

    def _check_without_docs(
        self,
        query: str,
        answer: str,
    ) -> QualityCheckResult:
        """无文档时检测幻觉"""
        from prompts import quality_check_no_docs
        from graph import llm_with_timeout

        messages = quality_check_no_docs(query=query, answer=answer)
        ...
        try:
            ...
            for line in content.split("\n"):
                ...
                if line.startswith("{") and line.endswith("}"):
                    ...
                    error_types = [AnswerErrorType(e) for e in result.get("error_types", [])]
                    return QualityCheckResult(
        ...
        return QualityCheckResult(
            is_acceptable=False,
            error_types=[AnswerErrorType.HALLUCINATION],
            details=["无法验证答案准确性"],
            reasoning="检测失败，保守标记为有问题",
        )
...
class ControlledSelfRAG:
    ...
    想象去医院看病：
    1. 分诊台（路由）→ 判断你该去哪个科
    2. 挂号处（RAG）→ 拿出你的病历本和检查报告
    3. 检查室（Rerank）→ 用仪器分析哪份报告最重要
    4. 医生诊室（有用性判断）→ 看完报告，判断能得出什么结论
    5. 开药处（LLM）→ 开处方
    6. 药剂师核对（分级纠错）→ 检查处方和报告是否一致
    7. 取药回家（最终回答）
    """
    ...
    def __init__(
        ...
        参数说明：
        - retrieval_router: 检索路由器（默认为新建实例）
        - usefulness_checker: 有用性检查器（默认为新建实例）
        """
        from config import settings

        # 初始化核心节点
        self.router = retrieval_router or RetrievalRouter()
    ...
    def process(
        ...
        if self.correction_pipeline is None:
            from CorrectionPipeline import CorrectionPipeline
        ...
        if not self.enabled:
            return ControlledSelfRAGResult(
                retrieval_decision=RetrievalDecision.MUST_RETRIEVE,
        ...
        # 如果路由器决定"跳过检索" → 直接返回（不做 RAG）
        if route_result.decision == RetrievalDecision.SKIP_RETRIEVE:
            logger.info(
                "controlled_self_rag_skipped",
                extra={"reason": route_result.reason, "component": "controlled_self_rag"}
            )
            return ControlledSelfRAGResult(
                retrieval_decision=RetrievalDecision.SKIP_RETRIEVE,
        ...
        if not docs:
            try:
                # 优先使用 LlamaIndex RAG Pipeline（向量检索 + BM25 + RRF 融合）
                from rag import build_llama_rag_pipeline, LlamaRAGPipeline
                from rag import build_llama_rag_pipeline as _build_pipeline

                # 构建 Pipeline（带 ABAC 权限过滤）
                ...
                # 降级：尝试旧版 rag.py 接口
                try:
                    from rag import build_abac_filtered_retriever
                    ...
                    logger.error(
                        "abac_rag_retrieval_failed",
                        extra={"error": str(e), "component": "controlled_self_rag"}
                    )
                    return ControlledSelfRAGResult(
                        retrieval_decision=route_result.decision,
                        should_retrieve=True,
                        retrieved=False,
                        is_useful=False,
                        needs_escalation=True,
                        escalation_reason=f"检索失败: {str(e)}",
                    )

        # ─────────────────────────────────────────────────────────────────
        # 第三步：Rerank 精排（如有必要）
        # ─────────────────────────────────────────────────────────────────
        # 用 BGE-Rerank 模型对文档重新排序，保留最相关的 top5
        try:
            # 优先使用 LlamaIndex Reranker（ Cohere / SentenceTransformer）
            ...
            logger.warning(
                "controlled_self_rag_rerank_failed",
                extra={"error": str(e), "component": "controlled_self_rag"}
            )
        ...
        # ─────────────────────────────────────────────────────────────────
        # 检查检索到的文档是否足够有用
        filtered_docs, usefulness_result = self.usefulness_checker.check(query, docs)
...
class CorrectionPipeline:
    ...
    def correct(
        ...
        # NO_EVIDENCE 且没有文档 → 拒答
        if AnswerErrorType.NO_EVIDENCE in error_types and not docs:
            logger.info(
                "no_evidence_no_docs_rejecting",
                extra={"component": "correction_pipeline"}
            )
            return self._reject(answer, "未找到相关信息，无法给出准确答案")

        # NO_EVIDENCE 但有文档 → 重度纠错（重新生成）
        if AnswerErrorType.NO_EVIDENCE in error_types and docs:
            logger.info(
        ...
        if AnswerErrorType.FACTUAL_ERROR in error_types:
            ...
            # 如果文档看起来相关 → 中度纠错；否则 → 重度纠错
            if self._docs_relevant(query, docs):
    ...
    def _docs_relevant(self, query: str, docs: List[Dict[str, Any]]) -> bool:
        """判断文档是否与问题相关"""
        if not docs:
            return False
        # 检查文档中是否包含问题的关键词
        ...
        for doc in docs:
            ...
            overlap = len(query_keywords & set(doc_text))
            if overlap > 3:  # 简单判断：有 3 个以上字符重叠
                return True
        return True  # 默认认为相关
    ...
    def _level_3_heavy_correction(
        ...
        try:
            ...
            new_docs = pipeline.retrieve(query=query, top_k=50, use_rerank=False)
        except Exception as e:
            # 降级：使用旧版 rag.py
            try:
                from rag import build_abac_filtered_retriever
                retriever = build_abac_filtered_retriever()
                new_docs = retriever.retrieve(
                    query=query,
                    user_id="",
                    top_k=50,
                )
            except Exception:
                logger.error("level_3_retrieval_failed", extra={"error": str(e)})
                return self._reject(answer, "重新检索失败")

        if not new_docs:
            logger.warning("level_3_no_new_docs")
            return self._reject(answer, "未找到相关信息")

        # BGE 重排（使用 LlamaIndex Reranker）
        try:
            from reranker_llamaindex import get_llama_reranker
            reranker = get_llama_reranker()
            new_docs = reranker.rerank(query=query, documents=new_docs, top_k=5)
        except Exception as e:
            logger.warning("level_3_rerank_failed", extra={"error": str(e)})

        # 严格生成
        from prompts import strict_generate_lv3, format_docs
        from graph import llm_with_timeout

        docs_text = format_docs(new_docs)
        messages = strict_generate_lv3(query=query, docs_text=docs_text)
