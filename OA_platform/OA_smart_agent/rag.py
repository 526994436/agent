...
_pipeline: Optional[LlamaRAGPipeline] = None
...
def build_llama_rag_pipeline(
    ...
    """构建 LlamaIndex RAG Pipeline 单例"""
    global _pipeline
    if _pipeline is None or hybrid_config is not None:
        _pipeline = LlamaRAGPipeline(
...
def build_graph_filtered_retriever() -> LlamaRAGPipeline:
    """向后兼容：构建 RAG Pipeline"""
    ...
    return build_llama_rag_pipeline(hybrid_config=config)


build_abac_filtered_retriever = build_graph_filtered_retriever
build_hybrid_retriever = build_graph_filtered_retriever