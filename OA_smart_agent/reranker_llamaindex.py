...
2. SentenceTransformerReranker — 本地 GPU，支持 Cross-Encoder 家族（BAAI/bge-reranker-large）
3. MockReranker              — 关键词匹配降级方案（无模型时自动启用）

核心类：
- LlamaReranker            — 底层重排序封装，支持多种模型自动切换
- LlamaRerankerPipeline    — Pipeline 封装，支持 RRF 多路融合 + 精排

使用示例：

    from reranker_llamaindex import get_llama_reranker, LlamaRerankerPipeline

    # 方式1：直接使用重排序
    reranker = get_llama_reranker(use_cohere=True, api_key="...")
    results = reranker.rerank("年假如何计算", documents, top_k=5)

    # 方式2：Pipeline 模式（配合多路检索使用）
    pipeline = LlamaRerankerPipeline(reranker_type="sentence_transformer", top_k=5)
