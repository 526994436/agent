...
async def leave_request(
    ...
    try:
        result = await asyncio.to_thread(
            ...
            lambda: asyncio.run(_http_call_with_retry(url, payload, headers)),
        )
        return {
            "success": True,
            "request_id": request_id,
...
async def rag_retrieve(
    ...
    try:
        ...
        if result.retrieval_decision == RetrievalDecision.SKIP_RETRIEVE:
            return {
                "success": True,
        ...
        if not result.should_retrieve:
            return {
                "success": True,
                "should_retrieve": False,
                "answer": result.answer or "这个问题我暂时无法回答，建议联系 HR 部门。",
                "docs": [],
                "message": "不需要检索",
            }
