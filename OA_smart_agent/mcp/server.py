...
# ─────────────────────────────────────────────
@mcp.tool()
async def onboarding_start_workflow(
    candidate_name: str,
    department: str,
    position: str,
    entry_date: str,
    ...
    该工具不会同步等待流程完成，而是：
    1. 在 PostgreSQL 中创建工作流实例（状态 = OFFER_PENDING）
    2. 异步发送 offer letter（通过 Celery 任务）
    3. 立即返回 instance_id，后续由外部事件驱动状态流转

    状态流转：
        OFFER_PENDING  →  offer_confirmed  →  OFFER_CONFIRMED
        OFFER_CONFIRMED  →  first_checkin  →  FIRST_CHECKIN  →  COMPLETED

    外部系统需在候选人确认后调用 POST /api/workflow/events（event=offer_confirmed），
    在入职打卡后调用 POST /api/workflow/events（event=first_checkin）。
    ...
    返回:
        instance_id: 工作流实例 ID（用于查询状态和事件追溯）
        state: 当前状态
        status_url: 状态查询地址
    """
    ...
    try:
        with get_session() as db:
            instance = OnboardingInstance(
                ...
                device_type=device_type,
                course_list=course_list,
                state=OnboardingState.OFFER_PENDING,
                current_step_detail="已创建工作流，等待发送 offer letter",
            )
            db.add(instance)
            db.commit()
    except Exception as e:
        return {
            "success": False,
            "message": f"创建工作流实例失败: {e}",
        }

    # 异步发送 offer letter（不阻塞，立即返回）
    try:
        from ..workflow.tasks import trigger_workflow_action
        trigger_workflow_action.delay(
            instance_id,
        ...
        logger.warning(
            "workflow_action_dispatch_warning",
    ...
    return {
        ...
        "state": OnboardingState.OFFER_PENDING.value,
        "message": f"入职工作流已启动，等待候选人确认 offer（instance_id: {instance_id}）",
        "status_url": f"/api/workflow/status/{instance_id}",
    }
...
java_backend_tools = [
    ...
    {
        "name": "onboarding_start_workflow",
        "description": "启动入职工作流（事件驱动、异步执行）：创建工作流实例，异步发送 offer letter，立即返回 instance_id，后续由外部事件驱动状态流转。",
        "input_schema": {
            "type": "object",
            "properties": {
                "candidate_name": {"type": "string", "description": "候选人姓名"},
