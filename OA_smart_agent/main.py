...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # 获取数据库连接地址
        url = settings.postgres_checkpointer_url
        if not url:
        ...
        # 初始化工作流数据库表
        try:
            from workflow import init_workflow_db
            init_workflow_db()
            logging.info("工作流数据库表初始化成功", extra={"component": "startup"})
        except Exception as e:
            logging.warning("工作流数据库表初始化失败", extra={"error": str(e), "component": "startup"})
...
app = FastAPI(
    ...
    - 动作执行：通过人工确认机制执行操作类请求
    - 入职工作流：事件驱动的入职全流程自动化（offer 发送 → 账号开通 → IT 设备 → 培训课程）
...
@mock_router.post("/api/onboarding/offer-letter")
async def mock_offer_letter(request: MockExecuteRequest) -> dict:
    params = request.params
    logging.info("收到入职通知书请求（Mock）", extra={"request_id": request.request_id, "params": params})
    return {
        "success": True,
        "message": f"入职通知书已生成并发送给 {params.get('candidate_name', '候选人')}",
        "request_id": request.request_id,
    }


@mock_router.post("/api/onboarding/oa-account")
async def mock_oa_account(request: MockExecuteRequest) -> dict:
    params = request.params
    logging.info("收到 OA 账号开通请求（Mock）", extra={"request_id": request.request_id, "params": params})
    return {
        "success": True,
        "message": f"OA 账号已开通，工号：{params.get('employee_id', '')}，邮箱：{params.get('email', '')}",
        "request_id": request.request_id,
        "account_id": f"oa_{params.get('employee_id', 'unk')}",
    }


@mock_router.post("/api/onboarding/it-equipment")
async def mock_it_equipment(request: MockExecuteRequest) -> dict:
    params = request.params
    logging.info("收到 IT 设备申请请求（Mock）", extra={"request_id": request.request_id, "params": params})
    return {
        "success": True,
        "message": f"IT 设备申请已提交，类型：{params.get('device_type', '未知')}，数量：{params.get('quantity', 1)}",
        "request_id": request.request_id,
        "equipment_id": f"eq_{request.request_id[:8]}",
    }


@mock_router.post("/api/onboarding/training")
async def mock_training(request: MockExecuteRequest) -> dict:
    params = request.params
    logging.info("收到培训课程添加请求（Mock）", extra={"request_id": request.request_id, "params": params})
    course_count = len(params.get("course_list", []))
    return {
        "success": True,
        "message": f"已为 {params.get('employee_name', '员工')} 添加 {course_count} 门培训课程",
        "request_id": request.request_id,
        "enrollment_id": f"enroll_{request.request_id[:8]}",
    }
...
workflow_router = APIRouter(prefix="/api/workflow", tags=["入职工作流"])
...
class WorkflowEventRequest(BaseModel):
    instance_id: str
    event: str  # offer_confirmed / first_checkin
...
async def start_workflow(request: StartWorkflowRequest):
    ...
    启动新的入职工作流（由 Agent 或 HR 调用）。
    内部实际调用 onboarding_start_workflow MCP 工具。
    """
    try:
        from mcp.server import onboarding_start_workflow
        result = await onboarding_start_workflow(
        ...
        logging.error(
            ...
            extra={"error": str(e), "component": "workflow"},
        )
        return {"success": False, "message": f"启动工作流失败: {e}"}


@workflow_router.post("/events")
async def receive_workflow_event(request: WorkflowEventRequest):
    ...
    接收外部系统事件，驱动工作流状态机流转。
    支持事件：
    - offer_confirmed: 候选人确认 offer
    - first_checkin: 新员工入职当天打卡
    """
    from workflow import OnboardingStateMachine
    from workflow.database import get_session
    from workflow.models import OnboardingInstance

    try:
        with get_session() as db:
            instance = db.query(OnboardingInstance).get(request.instance_id)
            if not instance:
                return {"success": False, "message": f"未找到工作流实例: {request.instance_id}"}

        sm = OnboardingStateMachine(instance)
        updated_instance = sm.process(request.event, request.payload or {})

        return {
            "success": True,
            "instance_id": updated_instance.id,
            "state": updated_instance.state.value,
            "current_step_detail": updated_instance.current_step_detail,
            "message": f"事件 '{request.event}' 已处理",
        }
        ...
        logging.error(
            ...
            extra={"instance_id": request.instance_id, "event": request.event, "error": str(e)},
        )
        return {"success": False, "message": f"处理事件失败: {e}"}


@workflow_router.get("/status/{instance_id}")
async def get_workflow_status(instance_id: str):
    """
    GET /api/workflow/status/{instance_id}

    查询工作流当前状态、各步骤完成情况和关联的 request_id。
    """
    from workflow.database import get_session
    from workflow.models import OnboardingInstance
    ...
    try:
        with get_session() as db:
            ...
            if not instance:
                return {"success": False, "message": f"未找到工作流实例: {instance_id}"}
            return {
                "success": True,
                **instance.to_dict(),
            }
...
async def list_workflows(
    ...
    列出工作流实例，支持按状态和员工 ID 过滤。
    """
    from workflow.database import get_session
    from workflow.models import OnboardingInstance, OnboardingState

    try:
        with get_session() as db:
            query = db.query(OnboardingInstance)
            if state:
                query = query.filter(OnboardingInstance.state == OnboardingState(state))
            if employee_id:
                query = query.filter(OnboardingInstance.employee_id == employee_id)
            instances = query.order_by(OnboardingInstance.created_at.desc()).limit(limit).all()
            return {
                "success": True,
                "count": len(instances),
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "workflow_start": "/api/workflow/start",  # 启动入职工作流
            "workflow_events": "/api/workflow/events",  # 接收工作流事件
            "workflow_status": "/api/workflow/status/{instance_id}",  # 查询工作流状态
