...
class ContractAuditRisk(BaseModel):
    """
    合同审计风险点 - 审计报告中识别的单个风险
    """
    risk_id: int = Field(..., description="风险点序号，从1开始")
    category: str = Field(..., description="风险类别，如"格式条款"、"违约责任缺失"、"管辖权条款"等")
    severity: Literal["high", "medium", "low"] = Field(..., description="严重程度：high/medium/low")
    clause_text: str = Field(..., description="涉及的合同条款原文")
    risk_description: str = Field(..., description="风险描述")
    legal_basis: Optional[str] = Field(None, description="相关法律依据，如《民法典》第XXX条")
...
class ContractAuditReport(BaseModel):
    ...
    contract_type: str = Field(..., description="合同类型，如"采购合同"、"劳动合同"、"租赁合同"等")
    counterparty: Optional[str] = Field(None, description="合同对方当事人")
    audit_passed: bool = Field(..., description="是否通过审计")
    audit_level: Literal["pass", "pass_with_conditions", "fail"] = Field(
        ..., description="审计结论：pass=通过, pass_with_conditions=有条件通过(需修改), fail=不通过"
    )
    overall_score: float = Field(..., description="综合评分 0-100")
    total_risks: int = Field(..., description="风险点总数")
    high_risks: int = Field(0, description="高风险数量")
    medium_risks: int = Field(0, description="中风险数量")
    low_risks: int = Field(0, description="低风险数量")
    risk_items: List[ContractAuditRisk] = Field(default_factory=list, description="风险点列表")
    summary: str = Field(..., description="审计摘要/总体评价")
    audit_suggestions: List[str] = Field(default_factory=list, description="整体修改建议列表")
    contract_struct: Optional[dict] = Field(None, description="合同结构化解析结果")
