...
def submit_chat_task(
    ...
    try:
        ...
        # 执行 LangGraph 工作流
        # 注意：在 ReAct 模式下，graph.invoke() 会在工具调用前自动中断（interrupt）
        # 这是 LangGraph 的原生 Human-in-the-loop 机制
        state_after_invoke = graph.invoke(input_state, config=config)
        
        # 检查是否是中断状态（工具调用前暂停）
        latest_state = graph.get_state(config)
        is_interrupted = latest_state.metadata.get("interrupted", False)
        
        # 获取回复内容（检查是否有 ToolMessage）
        ...
        # 检查是否有待执行的工具调用
        pending_tool_calls = [m for m in messages if hasattr(m, 'tool_calls') and m.tool_calls]
