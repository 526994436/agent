...
def submit_chat_task(
    ...
    try:
        ...
        # ─────────────────────────────────────────
        # 构建输入状态
        # 注意：checkpointer 已自动处理状态恢复，只需传入新消息
        # ─────────────────────────────────────────
        ...
        if requires_approval:
            ...
            # 遍历寻找真正需要审批的那个工具
            if tool_info and hasattr(tool_info, 'tool_calls') and tool_info.tool_calls:
                for tc in tool_info.tool_calls:
                    # 兼容字典和对象两种访问方式（LangChain 版本差异）
                    tc_name = tc.get("name") if isinstance(tc, dict) else getattr(tc, "name", "unknown")
                    tc_args = tc.get("args") if isinstance(tc, dict) else getattr(tc, "args", {})
                    
                    # 只要发现这个工具在“需审批名单”里，就把它提取出来用于前端展示
                    if tc_name in APPROVAL_REQUIRED_TOOLS:
                        draft_action_data = {
                            "action_type": tc_name,
                            "extracted_params": tc_args,
                            "payload": {
                                "action_type": tc_name,
                                "params": tc_args,
                            },
                            "confirmation_message": f"系统准备执行【{tc_name}】，请确认是否继续？",
                        }
                        break # 找到核心的动作类工具即可跳出
            
            # 兜底：万一没找到（极其罕见的异常状态）
            if not draft_action_data:
                draft_action_data = {
                    "action_type": "unknown",
                    "extracted_params": {},
                    "payload": {"action_type": "unknown", "params": {}},
                    "confirmation_message": "系统准备执行一项敏感操作，请确认是否继续？",
                }
