...
from canary import get_feature_flag_manager
from metrics import (
    record_llm_call,
...
class LLMWrapper:
    ...
    def with_structured_output(
        ...
        *,
        ...
        参数：
        - messages: 对话消息列表
        - output_class: Pydantic 模型类，用于结构化输出
        - model: 可选，指定模型名称
        ...
        try:
            ...
            response = self._invoke_structured_with_retry(structured_llm, messages)
            return response
        except Exception as e:
            logger.error(
                "LLM structured output 调用失败",
                extra={
                    "error": str(e),
                    "component": "llm",
                }
    ...
    def _invoke_structured_with_retry(self, structured_llm, messages, max_attempts: int = 3):
        """带重试的结构化输出调用"""
        for attempt in range(max_attempts):
            try:
                ...
                if attempt == max_attempts - 1:
                    raise
                logger.warning(
                    f"LLM structured output 重试 ({attempt + 1}/{max_attempts})",
                    extra={"error": str(e)}
                )
...
def rag_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        # ─────────────────────────────────────────────────────────────────
        # 灰度路由：根据开关决定使用新算法还是旧算法
        # ─────────────────────────────────────────────────────────────────
        fm = get_feature_flag_manager()
        use_new_rag = fm.is_enabled("new_rag_algorithm", user_id)

        logger.debug(
            "rag_gray_route",
            extra={
                "user_id": user_id,
                "use_new_rag": use_new_rag,
                "component": "rag",
            }
        )
        ...
        try:
            ...
            if use_new_rag:
                logger.warning(
                    "新 RAG 算法失败，尝试降级到旧算法",
                    extra={"error": str(e), "user_id": user_id, "component": "rag"}
                )
                return _fallback_to_old_rag(state, latest_message, multimodal_answer_addition)
            else:
                raise
...
def _fallback_to_old_rag(state: AgentState, query: str, multimodal_addition: str) -> AgentState:
    """
    降级到旧版 RAG 算法（LlamaIndex）

    当新 RAG 算法失败时，自动切换到旧版算法，保证服务可用性。
    """
    from langchain_core.messages import AIMessage
    from rag import LlamaRAGPipeline, build_hybrid_retriever

    try:
        logger.info(
            "执行旧版 RAG 降级流程",
            extra={"user_id": state.get("user_id", ""), "component": "rag"}
        )
        ...
        rag_message = AIMessage(content=answer)

        return {
            **state,
            "retrieved_docs": docs,
            "reranked_docs": docs,
            "retrieval_decision": "fallback_old_rag",
            "retrieval_quality_score": 0.5,  # 旧算法质量评分降低
            "retrieval_filtered_count": 0,
            "is_useful": bool(docs),
            "skipped_retrieval": False,
            "reflection_results": [],
            "needs_escalation": not bool(docs),
            "messages": [rag_message],
        }
        ...
        return {
            **state,
            "retrieved_docs": [],
            "reranked_docs": [],
            "retrieval_decision": "fallback_failed",
            "retrieval_quality_score": 0.0,
            "retrieval_filtered_count": 0,
            "is_useful": False,
            "skipped_retrieval": False,
