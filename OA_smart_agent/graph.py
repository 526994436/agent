...
│  │(+Tools) │     │  • contract_parse (合同解析)                  │     │
│  └──────────┘     │  • contract_audit (合同审计) ←── 需要审批  │     │
│       ▲           └──────────────────────────────────────────────┘     │
│       │                                   │                                │
│       │         (工具调用前自动中断)        ▼                                │
│       │                          ┌───────────────┐                        │
...
TOOLS_NO_APPROVAL = {
    ...
    "contract_parse",     # 合同结构解析（只读，不做操作）
}

# 需要审批的工具类型（动作类）
TOOLS_NEED_APPROVAL = {
    "contract_audit",   # 合同审计（涉及正式评审流程）
}
...
def build_agent_graph() -> StateGraph:
    ...
    审批规则：
    - 查询类工具（rag_retrieve、contract_parse）：无需审批，直接执行
    - 动作类工具（contract_audit）：需要审批
    """
