...
│  │          │     │                    MCP Tools                       │     │
│  │    LLM   │────▶│  ┌─────────────┐  ┌─────────────┐  ┌─────────┐ │     │
│  │(+Tools)  │     │  │ rag_retrieve│  │generate_off│  │create_oa│ │     │
│  └──────────┘     │  │ (知识库)    │  │er_letter  │  │account │ │     │
│       ▲           │  └─────────────┘  └─────────────┘  └─────────┘ │     │
│       │           └──────────────────────────────────────────────────┘     │
│       │                                   │                                │
│       │         (工具调用前自动中断)        ▼                                │
...
# 需要审批的工具类型（动作类）
TOOLS_NEED_APPROVAL = {
    "generate_offer_letter",    # 入职通知书生成
    "create_oa_account",       # OA 账号开通
    "apply_it_equipment",     # IT 设备申请
    "enroll_training_courses", # 培训课程添加
    "onboarding_start_workflow",  # 入职工作流启动（异步）
}
...
def should_continue(state: AgentState) -> Literal["llm", "tools", "tools_auto", END]:
    ...
    1. 如果最后一条消息包含 tool_calls → 判断工具类型
       ...
       - 动作类工具（generate_offer_letter 等）→ 中断等待审批
    ...
    if hasattr(last_message, "tool_calls") and last_message.tool_calls:
        ...
        for tool_call in last_message.tool_calls:
            ...
            # 只要发现一个需要审批的动作类工具，就标记为需要审批
            if tool_name not in TOOLS_NO_APPROVAL: 
                needs_approval = True
                break # 找到一个需审工具就足够触发中断了，跳出循环
...
def build_agent_graph() -> StateGraph:
    ...
    │  │(+Tools) │     │  • generate_offer_letter (入职通知书) ← 需审  │     │
    │  └──────────┘     │  • create_oa_account (OA 账号) ← 需审      │     │
    │       ▲           │  • apply_it_equipment (IT 设备) ← 需审     │     │
    │       │           │  • enroll_training_courses (培训) ← 需审     │     │
    │       │           └───────────────────────────────────────────────┘     │
    │       │                            │                                   │
    │       │           ┌────────────────┼────────────────┐                   │
    │       │           │                │                │                   │
    ...
    - 动作类工具（generate_offer_letter 等）：需要审批
    """
