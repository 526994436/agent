...
def execute_api_node(state: AgentState) -> AgentState:
    ...
    if approved is False:
        return {
            **state,
            "messages": [AIMessage(content="操作已被取消。如需重新发起，请输入新的请求。")],
            "interrupted": False,
            "execution_success": False,
            "execution_error": "用户拒绝审批",
        }
...
def build_agent_graph() -> StateGraph:
    ...
    # 1. 定义草稿填完后的去向
    def route_after_draft(state: AgentState) -> Literal["execute_api", END]:
        # 如果草稿填满了，意味着准备好要执行了，我们把它导向 API 执行节点
        if state.get("requires_approval", False):
            return "execute_api"
        # 如果没填满（还需要继续问用户），就暂停这一轮对话
        return END

    # 2. 替换掉原来的 graph.add_edge("draft_action", END)
    graph.add_conditional_edges(
        "draft_action",
        route_after_draft
    )
    ...
    # 3. 别忘了把 API 执行完的线连上
    graph.add_edge("execute_api", "generate_response")
