...
class MilvusIndexManager:
    ...
    def as_retriever(
        ...
        """
        返回带原生 Milvus Expr ABAC 过滤的 retriever。
        ...
        # 通过 vector_store_kwargs 强行将 Milvus Expr 透传到 MilvusVectorStore.search()
        expr = self.build_abac_expr(abac_depts or [], abac_projects or [])
        if expr:
            kwargs["vector_store_kwargs"] = {"expr": expr}

        kwargs["vector_store_query_mode"] = "sparse" if use_sparse else "default"
