...
class MilvusIndexManager:
    ...
    def process_document_incrementally(
        ...
        get_old_chunks_info: Callable[[str], Dict[str, str]],
        save_chunks_info: Callable[[str, List[Chunk]], None],
        delete_chunks_info: Callable[[str, List[str]], None] = None,
    ) -> Dict[str, Any]:
        """
        增量处理文档（幂等更新）

        核心逻辑：
        1. 对比新旧 Chunk 的 version_hash，计算差异
        2. 只对新增/修改的 Chunk 调用 Embedding API
        3. 先删后加，保证向量库与 Chunk 信息一致

        Args:
            document: 文档对象（用于获取 doc_id）
            new_chunks: 切块后的新 Chunk 列表
            get_old_chunks_info: 回调函数，从传统数据库拉取旧 Chunk 记录
                                 格式：{chunk_id: version_hash}
            save_chunks_info: 回调函数，保存新的 Chunk 信息到传统数据库
            delete_chunks_info: 可选回调函数，删除旧 Chunk 信息
        ...
        Returns:
            包含处理结果的字典：
            {
                ...
                "full_reindex_suggested": bool,
            }
        """
        from .incremental_updater import IncrementalUpdater

        doc_id = document.doc_id
        logger.info(f"开始增量处理文档: {doc_id}")

        # 1. 拉取历史记录
        old_chunks_info = get_old_chunks_info(doc_id)
        is_first_upload = not old_chunks_info

        if is_first_upload:
            logger.info(f"文档 {doc_id} 首次上传，执行全量写入")
            result = self.insert_chunk_objects(new_chunks)
            save_chunks_info(doc_id, new_chunks)
            return {
                "doc_id": doc_id,
                "added_count": result,
                "deleted_count": 0,
                ...
                "full_reindex_suggested": False,
            }

        # 2. 计算增量
        updater = IncrementalUpdater()

        if updater.is_full_reindex_needed(old_chunks_info, new_chunks):
            logger.info(f"文档 {doc_id} 变更比例过高，建议全量重索引")
            # 全量重索引：删除旧数据，重新写入
            self.delete_chunks_by_doc_id(doc_id)
            result = self.insert_chunk_objects(new_chunks)
            save_chunks_info(doc_id, new_chunks)
            return {
                "doc_id": doc_id,
                "added_count": result,
                "deleted_count": len(old_chunks_info),
                "kept_count": 0,
                "is_first_upload": False,
                "full_reindex_suggested": True,
            }

        # 3. 增量更新
        diff = updater.compute_diff(old_chunks_info, new_chunks)

        # 先删后加
        if diff.chunk_ids_to_delete:
            self.delete_chunks(diff.chunk_ids_to_delete)
            if delete_chunks_info:
                delete_chunks_info(doc_id, diff.chunk_ids_to_delete)
            logger.info(f"删除了 {len(diff.chunk_ids_to_delete)} 个旧 Chunk")

        if diff.chunks_to_add:
            result = self.insert_chunk_objects(diff.chunks_to_add)
            save_chunks_info(doc_id, diff.chunks_to_add)
            logger.info(f"新增/更新了 {result} 个 Chunk")
        else:
            result = 0
        ...
        return {
            "doc_id": doc_id,
            "added_count": result,
            "deleted_count": diff.delete_count,
            "kept_count": diff.keep_count,
            "is_first_upload": False,
            "full_reindex_suggested": False,
        }
