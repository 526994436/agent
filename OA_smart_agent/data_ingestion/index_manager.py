...
class MilvusIndexManager:
    ...
    def _get_vector_store(self, overwrite: bool = False) -> Any:
        ...
        if self._vector_store is None or overwrite:
            ...
            self._vector_store = MilvusVectorStore(
                ...
                dim=self.dimension,
                overwrite=overwrite,
                # 启用 sparse vector 支持（Milvus 2.4+ BM25 风格检索）
                enable_sparse=True,
                hybrid_search=True,
            )
    ...
    def build_abac_filter(
        ...
        """
        构建 ABAC MetadataFilters（用于向量检索 pre-filter）。
        ...
        使用 Milvus Expr 原生语法（array_contains 精确匹配数组字段），
        避免 JSON 字符串包含匹配的不准确性。
        底层 MilvusVectorStore 会将字符串值识别为 MilvusExpr 直接下发。
        """
        from pymilvus import ExprEvaluator

        parts: List[str] = []

        if user_depts:
            # array_contains 精确匹配 allowed_depts 数组字段
            dept_conditions = " || ".join(
                f'array_contains(allowed_depts, "{d}")' for d in user_depts
            )
            parts.append(f'(allowed_depts == "") || ({dept_conditions})')

        if user_projects:
            proj_conditions = " || ".join(
                f'array_contains(allowed_projects, "{p}")' for p in user_projects
            )
            parts.append(f'(allowed_projects == "") || ({proj_conditions})')
        ...
        # 用 ExprEvaluator 校验表达式安全性（防注入）
        try:
            ExprEvaluator.validate_expression(expr)
        except Exception as e:
            logger.warning(f"Milvus Expr 校验失败: {e}，跳过过滤")
            return MetadataFilters(filters=[], condition="or")

        logger.debug(f"ABAC Milvus Expr: {expr}")

        # MilvusVectorStore 会识别字符串值并作为 MilvusExpr 下发
    ...
    def _sparse_retrieve(self, query: str, top_k: int) -> List[Any]:
        """
        执行 Sparse Vector 检索（BM25 风格，Milvus 2.4+ 内置）。

        使用 MilvusVectorStore 的稀疏向量能力，结合 jieba 分词，
        实现关键词匹配检索，无需维护独立的 BM25Retriever。
        """
        try:
            vector_store = self._get_vector_store()
            ...
            sparse_results = vector_store.query(query_obj)
            logger.info(f"Sparse 检索: '{query[:30]}...' 返回 {len(sparse_results)} 条")
            return sparse_results
        except Exception as e:
            logger.error(f"Sparse 检索失败: {e}")
            return []

    def _get_bm25_sparse_retriever(self, top_k: int = 100) -> Any:
        """
        获取 Sparse Retriever（Milvus 2.4+ BM25 风格）。

        通过 MilvusVectorStore 配置 hybrid_search=True 后，
        可以直接使用 as_retriever 返回支持 sparse 的 retriever。
        """
        index = self._get_index()
    ...
    def as_retriever(
        ...
        参数：
            use_sparse: 若为 True，返回纯 sparse（BM25）retriever；
                        若为 False，返回 dense retriever；
                        混合检索由 LlamaRAGPipeline 内部协调。
        """
        index = self._get_index()
        ...
        # 根据检索模式选择 vector_store_query_mode
        if use_sparse:
