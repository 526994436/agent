...
class QueryRouter:
    ...
    def route(self, query: str, intent: Optional[QueryIntent] = None) -> QueryResult:
        ...
        if intent.query_type == QueryType.STATISTICAL:
            ...
            return self._execute_mixed_query(query, intent)
    ...
    def _execute_mixed_query(self, query: str, intent: QueryIntent) -> QueryResult:
        """执行混合查询"""
        stat_result = self._execute_statistical_query(query, intent)
        semantic_result = self._execute_semantic_query(query, intent)

        combined_answer = f"{stat_result.answer}\n\n{semantic_result.answer}"

        return QueryResult(
            query_type=QueryType.MIXED,
            answer=combined_answer,
            data=stat_result.data,
            source="postgresql+vector_db",
        )
...
def route_query(query: str) -> QueryResult:
    """便捷函数：根据查询类型路由执行"""
