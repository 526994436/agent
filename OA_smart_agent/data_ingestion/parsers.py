...
│ 格式      │ 解析方式                                                    │
├──────────┼─────────────────────────────────────────────────────────────┤
│ PDF       │ Unstructured.io partition_pdf (strategy="hi_res")          │
│ DOCX/DOC  │ DocxReader                                                 │
...
class BaseParser(ABC):
    ...
    def _read_file_with_fallback(self, file_path: str, error_msg: str) -> str:
        """用 FlatReader 读取文件，失败则直接读取（utf-8 → gbk）"""
        try:
...
class PDFParser(BaseParser):
    """
    PDF 解析器 - 基于 Unstructured.io

    使用 Unstructured.io 的 partition_pdf 函数，strategy="hi_res" 
    是专门为大语言模型和 RAG 场景设计的高分辨率解析策略：
    - 自动检测文档布局结构（标题、段落、表格、图像等）
    - 优先使用多模态模型（如有）识别复杂元素
    - 保持元素的层级关系和语义结构
    """

    def parse(self, file_path: str) -> Tuple[str, List[ChunkMetadata]]:
        elements = self._partition_with_unstructured(file_path)
        if not elements:
            logger.error(f"PDF 解析失败（Unstructured.io）：{file_path}")
            return "", []
        ...
        def flush_page():
            nonlocal current_page, page_buffer, page_element_count
            if page_buffer:
                page_content = "\n\n".join(page_buffer)
                markdown_parts.append(f"## 第 {current_page} 页\n\n{page_content}")
        ...
        for element in elements:
            element_text = element.text.strip() if hasattr(element, 'text') else str(element).strip()
            if not element_text:
                continue
            ...
            # 根据元素类型添加 Markdown 格式
            element_type = type(element).__name__
            if element_type == "Title":
                page_buffer.append(f"### {element_text}")
            elif element_type == "Table":
                # 表格保留原始格式，添加标记便于后续处理
                page_buffer.append(f"<table>\n{element_text}\n</table>")
            elif element_type == "Image":
                page_buffer.append(f"![image]({element_text})")
            elif element_type == "ListItem":
                page_buffer.append(f"- {element_text}")
            elif element_type == "NarrativeText":
                page_buffer.append(element_text)
            elif element_type == "CompositeElement":
                # 复合元素直接添加
                page_buffer.append(element_text)
            else:
                # 其他类型元素（Header, Footer, PageBreak 等）直接添加
                page_buffer.append(element_text)
    ...
    def _partition_with_unstructured(self, file_path: str) -> List[Any]:
        """
        使用 Unstructured.io partition_pdf 解析 PDF
        ...
        try:
            ...
            logger.error(
                "Unstructured.io 未安装，请执行: pip install unstructured[pdf] "
                "或 pip install "unstructured[pptx]" (如需图表支持)"
            )
            return []

        try:
            # 使用 hi_res 策略 - 专为 LLM/RAG 优化的高分辨率解析
            elements = partition_pdf(
                filename=file_path,
                strategy="hi_res",
                infer_table_structure=True,  # 启用表格结构识别
                extract_images_in_pdf=True,  # 提取 PDF 中的图像
                # chunking_strategy="by_title" 可选：按标题分块
            )
            return elements
        except Exception as e:
            logger.error(f"Unstructured.io partition_pdf 失败: {e}")
            return []
...
class MarkdownParser(BaseParser):
    ...
    def _read_file(self, file_path: str) -> str:
        return self._read_file_with_fallback(file_path, "读取 Markdown 文件失败")
...
class TextParser(BaseParser):
    ...
    def _read_file(self, file_path: str) -> str:
        return self._read_file_with_fallback(file_path, "读取文本文件失败")
